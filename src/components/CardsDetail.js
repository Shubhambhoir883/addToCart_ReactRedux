...
import { DLT,dataAddData,REMOVE } from '../redux/actions/addData'
...
const CardsDetail = () => {
  ...
  const getdata = useSelector((state)=> state.reduceData.allData);
  // console.log(getdata);


  const compare = ()=>{
    let comparedata = getdata.filter((ele)=>{
      return ele.id == id;
    });
    setData(comparedata);
